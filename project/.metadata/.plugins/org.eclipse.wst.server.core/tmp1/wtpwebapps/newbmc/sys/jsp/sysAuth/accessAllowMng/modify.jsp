...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        showAllowDateBox('${result.allowDateUseYn}');
        ...
        $("#submitBtn").click(function() {
            var options = {
                url: "${pageContext.request.contextPath}/sys/totalAdminMng/siteAccessMng/modifyProc.do",
                type: "post",
...
<!-- 내용시작 -->
<h2>${menuName}</h2>
<form id="frm" name="frm"  method="post" action="${pageContext.request.contextPath}/sys/totalAdminMng/siteAccessMng/modifyProc.do">
    <table class="tableSt_write">
        ...
        <tr>
            ...
            <td>
                <input type="radio" id="allowDateUseY" name="allowDateUseYn" value="Y" onclick="showAllowDateBox('Y');" <c:if test="${result.allowDateUseYn eq 'Y'}">checked</c:if>/><label for="allowDateUseY">사용</label>
                <input type="radio" id="allowDateUseN" name="allowDateUseYn" value="N" onclick="showAllowDateBox('N');" <c:if test="${result.allowDateUseYn eq 'N'}">checked</c:if>/><label for="allowDateUseN">무제한</label>
                <div id="allowDate" class="mT10">
                    <input type="text" id="allowDateStS" name="allowDateStS" class="input100" value="<fmt:formatDate value="${result.allowDateSt}" pattern="yyyy-MM-dd" />" readonly /> ~ <input type="text" id="allowDateEndS" name="allowDateEndS" class="input100" value="<fmt:formatDate value="${result.allowDateEnd}" pattern="yyyy-MM-dd" />" readonly/>
                </div>
            </td>
        </tr>
        <tr>
            ...
            <td>
                <input type="radio" id="useY" name="useYn" value="Y" <c:if test="${result.useYn == 'Y'}">checked</c:if> /><label for="useY">허가</label>
                <input type="radio" id="useN" name="useYn" value="N" <c:if test="${result.useYn == 'N'}">checked</c:if>/><label for="useN">차단</label>
            </td>
        </tr>
        </tbody>
    </table>
