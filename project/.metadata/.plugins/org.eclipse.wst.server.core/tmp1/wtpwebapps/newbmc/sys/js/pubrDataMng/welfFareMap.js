...
$(document).ready(function() {
    ...
    function initMapHeader() {
        ...
        headerHTML += '<th scope="row">구분</th>';
        headerHTML += '<th scope="row">시설명</th>';
        headerHTML += '<th scope="row">종별</th>';
    ...
    function getMapData(curPage) {
        $.getJSON(context + "/sys/pubcDataMng/dataSet/getWelfFareData.do",
            {curMapPage: curPage},
            function(j) {
                if(j.none != 'none') {
                    var data = "";
                    ...
                    for (var i = 0; i < j.length; i++) {
                        data += "<tr>";
                        data += "<td>" + (i + 1) + "</td>";
                        data += "<td>" + j[i].sGubun + "</td>";
                        data += "<td><a href='#' onclick='setDetailMarker("+j[i].latitude+","+ j[i].longitude +"); return false;'>" + j[i].sName + "</a></td>";
                        data += "<td>" + j[i].sKind + "</td>";
                        data += "<td>" + j[i].sRoad + "</td>";
                        data += "<td>" + j[i].sTel + "</td>";
    ...
    function setMapPaging(curPage) {
        $.getJSON(context + "/sys/pubcDataMng/dataSet/getWelfFarePaging.do",{curSheetPage: curPage},
            function(data) {
