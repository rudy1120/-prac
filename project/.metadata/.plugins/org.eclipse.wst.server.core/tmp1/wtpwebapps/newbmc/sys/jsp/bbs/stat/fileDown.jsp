...
<%
	 ...
	 */
%>
<c:set var="listUrl" value="${_context}/sys/bbs/stat/fileDown/list.do"/>
<c:set var="downUrl" value="${_context}/sys/bbs/stat/fileDown/downProc.do"/>

<h2>게시글 첨부파일 다운로드 건수 통계</h2>
<form:form commandName="searchVO" name="list" id="list" action="${listUrl}?mId=${menuVO.mId}" methodParam="post" onsubmit="return validator('#list');" htmlEscape="false">
	...
	<div class="search_box">
		<div class="fL">
			...
			<div id="div_m" style="display: none;" class="mR7">
			...
			<div id="div_r" style="display: none;">
	...
	<table class="tableSt_list row_over">
		<caption>게시글 첨부파일 다운로드 통계 목록을 게시판 타입, 사이트명, 게시판명, 첨부파일 다운로드 건수 순서대로 안내하는 표입니다.</caption>
		<thead>
			<tr>
				<th scope="col" class="w10">게시판 타입</th>
				<th scope="col">[사이트명] 게시판명</th>
				<th scope="col" class="w15">첨부파일 다운로드 건수</th>
			</tr>
		</thead>
		<tbody>
			<tr class="total_td">
				<td colspan="2"><span class="blue bold">${searchSummary}</span> 전체</td>
				<td class="w13"><fmt:formatNumber value="${snippet.sumDownCnt}" pattern="###,###,###"/></td>
			</tr>
			<c:forEach var="element" items="${result}" varStatus="status">
				<tr>
					...
					<td><fmt:formatNumber value="${element.downCnt}" pattern="###,###,###"/></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
