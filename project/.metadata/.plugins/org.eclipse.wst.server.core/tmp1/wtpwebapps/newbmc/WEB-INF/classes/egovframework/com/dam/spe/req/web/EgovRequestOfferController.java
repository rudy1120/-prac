...
public class EgovRequestOfferController {
	...
	public String EgovRequestOfferList(
		...
		int totCnt = (Integer) egovRequestOfferVOService.selectRequestOfferListCnt(searchVO);
		paginationInfo.setTotalRecordCount(totCnt);
		model.addAttribute("paginationInfo", paginationInfo);

		// (지식전문가/지식사용자) 검사 및 설정
		HashMap hmParam = new HashMap();
		hmParam.put("speId", (String) loginVO.getUniqId());
	...
	public String EgovRequestOfferDetail(
		...
		if (sCmd.equals("del")) {
			...
			HashMap hmParam = new HashMap();
			hmParam.put("ansParents", (String) requestOfferVO.getKnoId());
		...
		if (!sCmd.equals("del")) {
			// 상세정보 불러오기
			RequestOfferVO requestOfferVOs = (RequestOfferVO) egovRequestOfferVOService.selectRequestOfferDetail(requestOfferVO);
			...
			List MapTeamList = (List) mapTeamService.selectMapTeamList(mapTeamVO);
	...
	public String EgovRequestOfferModifyActor(
		...
		if (sCmd.equals("save")) {
			...
			// 아이디 설정
			RequestOfferVO.setFrstRegisterId((String) loginVO.getUniqId());
			RequestOfferVO.setLastUpdusrId((String) loginVO.getUniqId());
			// 첨부파일 관련 ID 생성 start....
	...
	public String EgovRequestOfferRegistActor(
		...
		if (sCmd.equals("save") || sCmd.equals("reply")) {
			...
			if (sCmd.equals("reply") && egovRequestOfferVOService.selectRequestOfferSpeCheck(hmParam)) {
				requestOfferVO.setSpeId((String) loginVO.getUniqId());
				// 지식전문가 아니고 reply 일때
			} else if (sCmd.equals("reply")) {
				return "egovframework/com/dam/spe/req/EgovComDamRequestOfferRegist";
				// 일반사용자일때
			} else {
				requestOfferVO.setEmplyrId((String) loginVO.getUniqId());
			}
