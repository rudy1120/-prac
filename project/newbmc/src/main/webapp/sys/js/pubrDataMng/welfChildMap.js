...
$(document).ready(function() {
    ...
    function initMapHeader() {
        ...
        headerHTML += '<th scope="row">시설종별</th>';
        headerHTML += '<th scope="row">시설명</th>';
        headerHTML += '<th scope="row">주소</th>';
        headerHTML += '<th scope="row">전화번호</th>';
        headerHTML += '<th scope="row">정원</th>';
        headerHTML += '<th scope="row">위도</th>';
        headerHTML += '<th scope="row">경도</th>';
        headerHTML += '<th scope="row">비고</th>';
        headerHTML += '</tr>';
    ...
    function getMapData(curPage) {
        $.getJSON(context + "/sys/pubcDataMng/dataSet/getWelfChildData.do",
            {curMapPage: curPage},
            function(j) {
                if(j.none != 'none') {
                    var data = "";
                    ...
                    for (var i = 0; i < j.length; i++) {
                        data += "<tr>";
                        data += "<td>" + (i + 1) + "</td>";
                        data += "<td>" + j[i].cType + "</td>";
                        data += "<td><a href='#' onclick='setDetailMarker("+j[i].latitude+","+ j[i].longitude +"); return false;'>" + j[i].cName + "</a></td>";
                        data += "<td>" + j[i].cRoad + "</td>";
                        data += "<td>" + j[i].cTel + "</td>";
                        data += "<td>" + j[i].cInt + "</td>";
                        data += "<td>" + j[i].cBigo + "</td>";
    ...
    function setMapPaging(curPage) {
        $.getJSON(context + "/sys/pubcDataMng/dataSet/getWelfChildPaging.do",{curSheetPage: curPage},
            function(data) {
