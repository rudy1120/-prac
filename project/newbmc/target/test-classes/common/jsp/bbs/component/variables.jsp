...
<c:set var="isRootArticle" value="${empty bbsVO.bIdx || bbsVO.bLevel == '0'}" scope="request"/>
<c:set var="isNew" value="${empty searchVO.bTitle}" scope="request"/>
<c:set var="postUrl" value="${isNew ? 'writeProc' : 'modifyProc'}" scope="request"/>
<c:set var="user" value="${yh:getUser()}" scope="request"/>
