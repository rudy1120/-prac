...
<form:form commandName="partContVO" id="list" name="list" methodParam="post" onsubmit="$('#page').val(1);">
	<form:hidden path="page" />
		...
		<ul class="clFix">
			...
			<li> 
				...
				<a href="#" data-action="/bmc/participation/view.do?idx=${element.idx}" onclick="req.post(this); return false;">
					<div class="thumb">
						<figure>
...
<c:choose>
	<c:when test="${not empty user}">
		<!-- 본인인증 정보존재 -->
		<a href="/bmc/participation/read.do?mId=${menuVO.mId}">참여내역조회</a>
		
	</c:when>
	<c:otherwise>
		<!-- 본인인증 정보없음 -->
		<a href="/bmc/user/inRealName.do?mId=${menuVO.mId}&successUrl=/bmc/participation/read.do">참여내역조회</a>
	</c:otherwise>
</c:choose>
</div>
</div></div>
