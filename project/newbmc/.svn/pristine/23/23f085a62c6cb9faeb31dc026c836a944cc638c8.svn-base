		...
		<tbody>
			<c:forEach var="element" items="${result}" varStatus="status">
				...
				<tr>
					...
					<td class="taL">
						<a href="/sys/receipt/memberList.do?mId=${menuVO.mId}&idx=${element.idx}">${element.title}</a>
					</td>
					<td>
					...
					<td>
						<c:choose>
                            <c:when test="${element.gbn == 'A'}">기능인재 접수</c:when>
                            <c:when test="${element.gbn == 'B'}">기간제직원 접수</c:when>
                        </c:choose>
					</td>
					<td>
						<a href="#" data-action="/sys/receipt/modify.do" data-keyset="{'idx': ${element.idx}}" onclick="req.post(this); return false;" class="btn_swhite">수정</a>
						<a href="#" data-action="/sys/receipt/deleteProc.do?mId=${menuVO.mId}" data-keyset="{'idx': ${element.idx}}" onclick="req.ajax(this); return false;" class="btn_sblack">삭제</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
...
<div class="btn_boxR">
	<a href="#" data-action="/sys/receipt/write.do" onclick="req.post(this); return false;" class="btn_cyan">등록</a>
</div>
