...
$(document).on("orientationchange", function (e) {
	...
	//  status.text(i + '/' + (slick.$dots[0].children.length));
		status.eq(0).text('0'+i+' / ');
		status.eq(1).text('0'+(slick.$dots[0].children.length));
	});
	...
	$('.pause').click(function(){
		if($(this).hasClass('on')){
		 $(this).attr("title", "슬라이드 화면 멈추기");
		 $(this).removeClass('on');
		 slickElement.slick('slickPlay');
		}else{
		 $(this).attr("title", "슬라이드 화면 재생하기");
		 $(this).addClass('on'); 
	...
	$('.pagi-link').click(function(e){
		e.preventDefault();
		var sectionTop = $(this.hash);
		$('html, body').animate({
			scrollTop : $(sectionTop).offset().top-60
		},1000);
