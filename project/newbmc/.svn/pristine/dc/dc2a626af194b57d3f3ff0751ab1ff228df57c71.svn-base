...
//<![CDATA[
    function boardSubmit(){
        ...
        frm.page.value = 1;
        frm.action = '${pageContext.request.contextPath}/sys/stateExceptIPMng/list.do?mId=${curMid}';
        frm.submit();
    }
    ...
    function insertAccessAllow() {
        var frm = document.getElementById('frm');
        frm.action = '${pageContext.request.contextPath}/sys/stateExceptIPMng/write.do?mId=${curMid}';
        frm.submit();
    }

    function modifyAccessAllow(seq) {
        var frm = document.getElementById('frm');
        frm.action = '${pageContext.request.contextPath}/sys/stateExceptIPMng/modify.do?seq='+seq+'&mId=${curMid}';
        frm.submit();
    }
...
<h2>${menuName}</h2>
<form name="frm" id="frm" action="${pageContext.request.contextPath}/sys/stateExceptIPMng/list.do?mId=${curMid}" method="post">
        ...
        <c:if test="${not empty resultList}">
            <c:forEach var="result" items="${resultList}" varStatus="status">
                <tr>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${result.useYn == 'Y'}">제외</c:when>
                            <c:otherwise>미제외</c:otherwise>
                        </c:choose>
                    </td>
                    <td>
