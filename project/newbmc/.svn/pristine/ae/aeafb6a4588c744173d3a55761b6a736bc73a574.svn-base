...
<!-- 		</p> -->
	...
	<c:if test="${not empty result}">
		...
		<div class="btn_wrap">
			<c:if test="${type.gbn eq 'A'}">
				<input type="button" class="btn-blue" onclick="location.href='/bmc/receipt/view.do?idx=${type.idx}&mId=${menuVO.mId}'" value="취소"/>
			</c:if>
			<c:if test="${type.gbn eq 'B'}">
				<input type="button" class="btn-blue" onclick="location.href='/bmc/receipt/view.do?idx=${type.idx}&mId=${menuVO.mId}'" value="취소"/>
			</c:if>
