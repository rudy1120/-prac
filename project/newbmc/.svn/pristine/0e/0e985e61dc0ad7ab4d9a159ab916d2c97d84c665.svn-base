	...
	if(confirm("알림서비스를 철회하시겠습니까?")==true){
		$.ajax({
			...
			success : function(data) {
				alert(data);
				self.close();
				location.href = "/bmc/sub/delForm.do";
			},
			error:function(request,status,error){
				alert("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
			}
...
<body>
<h3 style="font-size:3em;margin-bottom:20px;" class="tit">웹진 구독서비스 철회</h3>
	...
	<div class="all_wrap">
		...
		<div class="bt_wrap">
			...
			<a href="/bmc/sub/delForm.do" class="bt bt3 on" title="구독 알림서비스 철회">구독 철회</a>
		</div>
