...
<%--
*********************************************************************
* 파 일 명 : sendMenuList.jsp
* 설       명 : 사이트 메뉴를 CMS 폴더로 전달하는 페이지
* 작   성  자 : 권태성
* 작   성  일 : 2016-06-13
*********************************************************************
--%>
...
$(document).ready(function(){
	...
	/**
	 *
	 * 메뉴 마다 개별적으로 CMS에 폴더등록을 요청하는 함수 입니다.
	 *
	 **/
	var depthSave = function () {
 ...
 */
		...
		$.ajax({
		    type : "POST",
		    url : "${cmsUrl}/linkage/addFolder?key=${sessionScope.cmsLoginKey}",
		    data : {
			    	'siteCode' : siteCode, 
		    ...
		    async: false,
//		    beforeSend : function(){
//		    	console.log("> send count : "+depthNowCount+ " / "+"folderName : "+folderName+" / folderCode : "+folderCode+" / parentCode : "+parentCode);
//		    },
		    ...
		    error : function(xhr, status, error) {
	            $("#depth"+depth+"_status_"+depthNowCount).attr("src","${pageContext.request.contextPath}/sys/AXJ/ui/arongi/images/dx-noti-caution.png");
	            $("#depth"+depth+"_status_"+depthNowCount).attr("title","y-SmartCMS 서버에 연결하지 못하였습니다.");
	            depthNowCount++;
		    },
		    complete : function(){
		    	if(depthNowCount <= totalCount){
		        	depthSave(depth, totalCount);
		        }else{
		        	if(depth <= 5){
		        		depth = depth+1;
...
<h2>메뉴 전송</h2>
<div class="mB10">
	<span class="info red">전체 메뉴가 한번에 CMS로 등록됩니다. <b>메뉴 전송후에 CMS에서 컨텐츠 등록을 하셔야 됩니다.</b></span><br/>
	<span class="tip"><b>에러 발생시 에러 아이콘 위에 마우스를 올리시면 메시지를 확인할 수 있습니다.</b></span>
</div>
